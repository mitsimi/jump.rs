import type { Device, DeviceFormData, WakeResponse, MacLookupResponse } from '../types/device';
...
export async function updateDevice(id: string, data: DeviceFormData): Promise<Device> {
  const payload = {
    name: data.name,
    mac_address: data.mac_address,
    ip_address: data.ip_address || null,
